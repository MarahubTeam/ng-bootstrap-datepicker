...
import { DL_DATE_TIME_DISPLAY_FORMAT, DL_DATE_TIME_INPUT_FORMATS } from '../core/dl-date-time-string-format';
...
export class DlDateTimeInputDirective<D> implements ControlValueAccessor, Validator {
  ...
  private _validator: any = Validators.compose([this._parseValidator, this._filterValidator]);
  private _onValidatorChange: () => void = () => { };
  private _value: D | any = undefined;
  ...
  @HostListener('input', ['$event.target.value']) _onInput(value: string | null | undefined): void {
    const testDate: any = value === null || value === undefined || value === ''
      ? undefined
